...
//int addSurfaceLoop(Mesh& m, int Nx, int Ny, int loopMode, double width, double height, double x, double y) 
  ...
  void onProcess(Graphics& g) override {
    ...
    g.pushMatrix();
    g.depthTesting(true);
    g.translate(timeposeX, timeposeY, -(15));
    g.rotate(a_rotate, Vec3f(0, 1, 1));
    g.rotate(b_rotate, Vec3f(1));    
    g.scale(0.1 + mAmpEnv() * 2, 0.1 + mAmpEnv() * 2, 0.03 + 0.1*mAmpEnv() );
 ...
 srand (time(NULL));
    ...
    timepose1 = (rand() % 4) + 2.0; 
    timeposeX = t*timepose1;
    timeposeY = z*timepose1;
...
void onProcess(Graphics& g) override {
    ...
    g.depthTesting(true);
    g.translate(0, 0, -30);
    g.rotate(0, Vec3f(0, 1, 1));
    g.rotate(0, Vec3f(1));    
    g.scale(5, 5, 10);
    g.color(RGB(0.7, 0.85, 0.75));
...
public:
  ...
  // Test: JC
  double timeposeX = 0.0;
  double timeposeY = 0.0;
...
// \param[in] numh		total number of harmonics
  ...
  void onProcess(Graphics &g) override
  {
    a += 0.29;
    b += 0.23;
    // timepose = helpe {r_handleTimePoseCenter(timepose);
    ...
    float scaling = getInternalParameterValue("amplitude") * 2;
    g.scale(scaling *10/timepp, scaling *10/timepp,scaling *10/timepp);
    g.color(HSV(getInternalParameterValue("modMul") / 20, getInternalParameterValue("carMul") / 20, 0.5 + getInternalParameterValue("attackTime")));
    g.draw(mMesh[shape]);
    g.popMatrix();
  }
...
public:
    ...
    virtual void onProcess(Graphics &g)
    {
        ...
        {
        g.translate(radius/4 * sin(timepose + phase) * 10/timepose, radius/4 * cos(timepose + phase)* 10/timepose, -25);
        }
        g.rotate(a_rotate, Vec3f(1, 1, 1));
        g.rotate(b_rotate, Vec3f(1));
         if (frequency < 500)
         {
        g.scale(1.5, 1.6, 1.47);
        g.scale(1.4, 1.5, 1.29);
         }
        else
        {
        g.scale(1/timepose * (2 - frequency/4000), 1/timepose * (2 - frequency/4000)*1.3, 1/timepose * (2 - frequency/4000)*1.1);
        g.scale(1/timepose * (2 - frequency/4000)*0.87, 1/timepose * (2 - frequency/4000)*0.9, 1/timepose * (2 - frequency/4000)*1.2);
        }
        //g.scale(3 + mAmpEnv() * 0.5, 3 + mAmpEnv() * 0.5, 5 + mAmpEnv());

        if (frequency < 500)
        {
          g.color(HSV(0,100,74));
          //g.color(HSV(0.9 + frequency / 10000 , 0.8 + mAmpEnv() * 0.4, 0.1));
        }
        else
       { 
        g.color(RGB(0 ,250, 250));
       }
        g.draw(mMesh[shape]);
        g.popMatrix();
...
public:
  ...
  virtual void onProcess(Graphics &g)
  {
    ...
    g.depthTesting(true);
    g.lighting(true);
    g.translate(Vec3f(2 * cos(timepose)* 10/timepose, sin(timepose)*5/timepose, -15));
    // g.translate(note_position + note_direction * timepose);
    g.rotate(0, Vec3f(0, 1, 0));
    g.rotate(0, Vec3f(1));
    g.scale(0.1 + mEnvStri() * 0.05, 0.05 + mEnvStri() * 0., 0.08);
    g.color(HSV(20, 100, 30));
    //   g.color(HSV(frequency / 1000, 0.5 + mEnvStri() * 0.1, 0.1 + 0.5 * mEnvStri()));
    g.draw(ball);
    g.popMatrix();
  }
...
public:
    ...
    b += a;
        ...
        float frequency = getInternalParameterValue("frequency");
        float amplitude = getInternalParameterValue("amplitude");
        float radius = frequency / 600;
        // Now draw
        g.pushMatrix();
        g.depthTesting(true);
        g.lighting(true);
        // g.translate(note_position);
        g.translate(rand() % 10 * (radius * sin(timepose)), rand() % 10 * (radius * cos(timepose)), -15);
        


//first randomized spot landing on randomized spot on the orbit
//second step, from that spot it goes around the orbit. 


        g.rotate(a, Vec3f(mCFEnv(), mBWEnv(), 0));
        g.rotate(b, Vec3f(mNoise()));
        //g.scale(mCFEnv()/ 2000, mBWEnv()/ 2000,  0.4 + 0.1*mNoise());
        g.scale(0.15 + 0.1*mNoise(), 0.15 + 0.1*mNoise(),  0.15 + 0.1*mNoise());
        g.color(HSV(0.6 + frequency / 10000, 0.6 + mOsc() * 0.3, 0.3 + 0.5*mNoise()));
        g.draw
        (mMesh);
        g.popMatrix();
    }
